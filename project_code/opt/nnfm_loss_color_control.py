...
class NNFMLoss(torch.nn.Module):
    ...
    def forward(
        ...
        # outputs_new = outputs_temp[0].permute(1, 2, 0).detach().cpu().numpy() * 255.0
        # output_gray = cv2.cvtColor(outputs_new, cv2.COLOR_BGR2GRAY)

        # im_output_gray = output_gray.astype(np.float32) / 255.0


        # print('im_output_gray', im_output_gray.shape)
        # im_output_gray = np.repeat(im_output_gray[..., np.newaxis], 3, -1)
        # print('im_output_gray', im_output_gray.shape)

        # im_output_gray_tensor = torch.tensor(im_output_gray)
        # im_output_gray_tensor = im_output_gray_tensor.permute(2, 0, 1).unsqueeze(0).to(device)

        # print('im_output_gray_tensor shape', im_output_gray_tensor.shape)

        # if True:
        #     imageio.imwrite(
        #         os.path.join('wenzhao', f"wenzhao_output_gray.png"),
        #         output_gray.astype(np.uint8),
        #     )
        
        # if True:
        #     imageio.imwrite(
        #         os.path.join('wenzhao', f"wenzhao_output_gray_3_channel.png"),
        #         np.clip(im_output_gray * 255.0, 0.0, 255.0).astype(np.uint8),
        #     )
        
        
        # styles_new = styles_temp[0].permute(1, 2, 0).detach().cpu().numpy() * 255.0
        # styles_gray = cv2.cvtColor(styles_new, cv2.COLOR_BGR2GRAY)

        # if True:
        #     imageio.imwrite(
        #         os.path.join('wenzhao', f"wenzhao_style_gray.png"),
        #         styles_gray.astype(np.uint8),
        #     )
        

        # im_style_gray = styles_gray.astype(np.float32) / 255.0

        # print('im_style_gray', im_style_gray.shape)
        # im_style_gray = np.repeat(im_style_gray[..., np.newaxis], 3, -1)
        # print('im_style_gray', im_style_gray.shape)

        # if True:
        #     imageio.imwrite(
        #         os.path.join('wenzhao', f"wenzhao_style_gray_3_channel.png"),
        #         np.clip(im_style_gray * 255.0, 0.0, 255.0).astype(np.uint8),
        #     )

        # im_style_gray_tensor = torch.tensor(im_style_gray)
        # im_style_gray_tensor = im_style_gray_tensor.permute(2, 0, 1).unsqueeze(0).to(device)
        # print('im_style_gray_tensor shape', im_style_gray_tensor.shape)
