...
class NNFMLoss(torch.nn.Module):
    ...
    def forward(
        ...
        # if True:
        #     styles1 = F.interpolate(styles1, scale_factor=float(1.3))
        ...
        if True:
            styles2 = F.interpolate(styles2, scale_factor=float(0.8))
        ...
        blocks_style_fine = [2] # [0, 1] #[2]
        blocks_style_fine_weight = [1] # [0.3, 1.7] more fine style
        blocks_style_fine.sort()

        scale_value_for_style1 = 1.5
        scale_value_for_style2 = 1
        ...
        with torch.no_grad():
            # need rescale style image to each layer
            gray_image_styles1 = F.interpolate(gray_image_styles1, scale_factor=float(scale_value_for_style1))
            gray_image_styles2 = F.interpolate(gray_image_styles2, scale_factor=float(scale_value_for_style2))
        ...
        if True: # calculate fine style
            ...
            for index, block in enumerate(blocks_style_coarse):
                ...
                if "nnfm_loss" in loss_names:
                    ...
                    # if epoch_id < 7:
                    #     loss_dict["nnfm_loss"] = loss_dict["nnfm_loss"] +  cos_loss(x_feats_coarse, target_feats)
                    # else:
                    # print('blocks_style_fine_weight[index]', blocks_style_coarse_weight[index])
        ...
        if True: # calculate coarse style
            ...
            # for block in blocks_style_fine:
                ...
                if "nnfm_loss" in loss_names:
                    ...
                    # print('blocks_style_fine_weight[index]', blocks_style_fine_weight[index])
                    loss_dict["nnfm_loss"] += cos_loss(x_feats_fine, target_feats) * blocks_style_fine_weight[index]
