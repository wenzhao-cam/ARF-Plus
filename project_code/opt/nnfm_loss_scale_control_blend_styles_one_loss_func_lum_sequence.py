...
class NNFMLoss(torch.nn.Module):
    ...
    def forward(
        ...
        if True:
            styles2 = F.interpolate(styles2, scale_factor=float(0.5))
        ...
        blocks_style_coarse = [2, 3] #[2]
        blocks_style_coarse_weight = [1, 0.3]  # [1.7. 0,3] more coarse style
        blocks_style_coarse.sort()

        # blocks_style_fine = [0, 1] #[2]
        # blocks_style_fine_weight = [0.3, 0.7] # [0.3, 1.7] more fine style
        blocks_style_fine = [2] #[2]
        blocks_style_fine_weight = [2] # [0.3, 1.7] more fine style
            ...
            for index, block in enumerate(blocks_style_coarse):
                ...
                if "nnfm_loss" in loss_names:
                    ...
                    if epoch_id <= 3:
                        ...
                        loss_dict["nnfm_loss"] +=  cos_loss(x_feats_coarse, target_feats) * blocks_style_coarse_weight[index] * 0.2
                    # loss_dict["nnfm_loss"] +=  cos_loss(x_feats_coarse, target_feats) * 3 / len(blocks_style_coarse) * blocks_style_coarse_weight[index]
        # if True:
        if epoch_id > 3: # calculate coarse style
