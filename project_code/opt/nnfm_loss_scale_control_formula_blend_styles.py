...
def argmin_cos_distance(a, b, center=False):
    ...
    for i in range(0, a.shape[-1], loop_batch_size):
        ...
        # torch.argmin Returns the indices of the minimum value(s) of the flattened tensor or along a dimension
...
class NNFMLoss(torch.nn.Module):
    ...
    def forward(
        ...
        styles1,
        styles2,
        style1_vgg_layer_weight='0-1-0-0-0',
        style2_vgg_layer_weight='0-0-1-0-0',
        blocks=[
            2,
        ],
        ...
        print('style_vgg_layer', style_vgg_layer)
        print('style1_vgg_layer_weight', style1_vgg_layer_weight)
        print('style2_vgg_layer_weight', style2_vgg_layer_weight)

        blocks_style = style_vgg_layer.split('-') #small 4 
        blocks_style = [int(item) for item in blocks_style]
        blocks_style.sort()
        print('blocks_style', blocks_style)
        

        blocks_style1_weight = style1_vgg_layer_weight.split('-')
        blocks_style1_weight = [float(item) for item in blocks_style1_weight]
        print('blocks_style1_weight', blocks_style1_weight)

        blocks_style2_weight = style2_vgg_layer_weight.split('-')
        blocks_style2_weight = [float(item) for item in blocks_style2_weight]
        print('blocks_style2_weight', blocks_style2_weight)

        style1_img_scales = style1_img_scales.split('-')
        style1_img_scales = [float(item) for item in style1_img_scales]
        
        style2_img_scales = style2_img_scales.split('-')
        style2_img_scales = [float(item) for item in style2_img_scales]
        ...
        for block in blocks_style:
            all_layers_style += block_indexes[block]
        
        
        # print('all_layers', all_layers)
        # print('all_layers_stye', all_layers_style)

        x_feats_all_c = self.get_feats(outputs, all_layers)
        x_feats_all_s = self.get_feats(outputs, all_layers_style)
        
        # style_img_scales = [1, 1, 1, 1, 2]
        style_img_feats_arr = []

        with torch.no_grad():
            s_feats_all = self.get_feats(styles, all_layers_style)
            for index, scale_value in enumerate(style_img_scales):
                if scale_value != 1:
                    styles_ds = F.interpolate(styles, scale_factor=float(scale_value))
                    print('styles_downsample', styles_ds.shape)
                    s_feats_all_new = self.get_feats(styles_ds, all_layers_style)
                    style_img_feats_arr.append(s_feats_all_new)
                else:
                    style_img_feats_arr.append(s_feats_all)

            if "content_loss" in loss_names:
                content_feats_all = self.get_feats(contents, all_layers)

        print('style_img_feats_arr_len', len(style_img_feats_arr))

        ix_map_c = {}
        for a, b in enumerate(all_layers):
            ix_map_c[b] = a
        print('ix_map_c', ix_map_c)

        ix_map_s = {}
        for a, b in enumerate(all_layers_style):
            ix_map_s[b] = a
        print('ix_map_s', ix_map_s)
        

        loss_dict = dict([(x, 0.) for x in loss_names])

        # for block in blocks:
        # here we should change the self-defined block_styles
        print('zzzz-blocks_style-len', len(blocks_style))

        for index, block in enumerate(blocks_style):
            layers = block_indexes[block]
            print('cal style layers', layers)

            x_feats = torch.cat([x_feats_all_s[ix_map_s[ix]] for ix in layers], 1)
            # x_gray_feats = torch.cat([x_gray_feats_all[ix_map[ix]] for ix in layers], 1)
            s_feats = torch.cat([style_img_feats_arr[index][ix_map_s[ix]] for ix in layers], 1)

            if "nnfm_loss" in loss_names and blocks_style_weight[index] > 0:
                print('----')
                print('index', index)
                print('weight', blocks_style_weight[index])
                print('----')

                target_feats = nn_feat_replace(x_feats, s_feats)
                # loss_dict["nnfm_loss"] += cos_loss(x_feats, target_feats)
                loss_dict["nnfm_loss"] += cos_loss(x_feats, target_feats) * blocks_style_weight[index]
                # loss_dict["nnfm_loss"] = loss_dict["nnfm_loss"] * 3 / len(blocks_style)
        ...
        for block in blocks:
            layers = block_indexes[block]
            print('cal content layers', layers)

            x_feats = torch.cat([x_feats_all_c[ix_map_c[ix]] for ix in layers], 1)

            if "content_loss" in loss_names:
                content_feats = torch.cat([content_feats_all[ix_map_c[ix]] for ix in layers], 1)
                loss_dict["content_loss"] += torch.mean((content_feats - x_feats) ** 2)
