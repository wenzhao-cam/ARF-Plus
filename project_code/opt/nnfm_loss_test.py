...
def argmin_cos_distance(a, b, center=False):
    ...
    # a.shape[-1] 11844
    print('loop_batch_size', loop_batch_size)
    # loop_batch_size 8443

    for i in range(0, a.shape[-1], loop_batch_size):
        a_batch = a[..., i : i + loop_batch_size]
        print('a_batch.shape', a_batch.size())
        # a_batch.shape torch.Size([1, 768, 8443])
        # a_batch.shape torch.Size([1, 768, 3401])

        a_batch_norm = ((a_batch * a_batch).sum(1, keepdims=True) + 1e-8).sqrt()
        print('a_batch_norm.shape', a_batch_norm.size())
        # a_batch_norm.shape torch.Size([1, 1, 8443])
        # a_batch_norm.shape torch.Size([1, 1, 3401])
        ...
        print('d_mat.shape', d_mat.size())
        # d_mat.shape torch.Size([1, 8443, 11844])
        # d_mat.shape torch.Size([1, 3401, 11844])

        z_best_batch = torch.argmin(d_mat, 2)
        print('z_best_batch.shape', z_best_batch.size())
        # z_best_batch.shape torch.Size([1, 8443])
        # z_best_batch.shape torch.Size([1, 3401])
...
def nn_feat_replace(a, b):
    n, c, h, w = a.size()
    n2, c, h2, w2 = b.size()
    # nn_feat_replace(x_feats, s_feats)
    print('a.shape', a.size())
    # a.shape torch.Size([1, 768, 94, 126])
    print('b.shape', b.size())
    # b.shape torch.Size([1, 768, 94, 126])

    assert (n == 1) and (n2 == 1)

    a_flat = a.view(n, c, -1)
    print('a_flat.shape', a_flat.size())
    # a_flat.shape torch.Size([1, 768, 11844])

    b_flat = b.view(n2, c, -1)
    print('b_flat.shape', b_flat.size())
    # b_flat.shape torch.Size([1, 768, 11844])
    ...
    for i in range(n):
        ...
        print('z_best.shape', z_best.size())
        #z_best.shape torch.Size([1, 11844])

        z_best = z_best.unsqueeze(1).repeat(1, c, 1)

        print('z_best.shape', z_best.size())
        #z_best.shape torch.Size([1, 768, 11844])

        # torch-gather用于提取张亮中的特定元素
        feat = torch.gather(b_ref, 2, z_best)
        z_new.append(feat)

    z_new = torch.cat(z_new, 0)
    print('z_new.shape zz', z_new.size())
    # z_new.shape zz torch.Size([1, 768, 11844])
    z_new = z_new.view(n, c, h, w)
    print('z_new.shape zzz', z_new.size())
    # z_new.shape zzz torch.Size([1, 768, 94, 126])
