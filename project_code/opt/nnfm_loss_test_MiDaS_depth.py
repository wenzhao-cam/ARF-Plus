...
# prepare depth pre-trained model - midas
...
class NNFMLoss(torch.nn.Module):
    ...
    def forward(
        ...
        for x in loss_names:
            assert x in ['nnfm_loss', 'content_loss', 'gram_loss', 'depth_loss']
        ...
        for block in blocks:
            ...
            # print('layers', layers)
            # print('x_feats_all[ix_map[ix]]', x_feats_all[ix_map[11]].shape)
            # print(x_feats_all[ix_map[11]])
            # print('x_feats_all[ix_map[ix]]', x_feats_all[ix_map[13]].shape)
            # print('x_feats_all[ix_map[ix]]', x_feats_all[ix_map[15]].shape)
            ...
            # print('s_feats.shape', s_feats.shape)

            # print('s_feats.shape[0[]', s_feats[0].shape)
            device = torch.device('cuda:0')
            ############### change by multiple mask #############
            mask_height = x_feats.shape[2]
            mask_width = x_feats.shape[3]
            mask_bath_size = x_feats.shape[1]

            mask = np.ones((mask_height, mask_width))
            mask[mask_height//2:, :] = 0.000001
            mask = torch.tensor(mask)
            mask = mask.unsqueeze(0).repeat(mask_bath_size, 1, 1)
            ...
            if "content_loss" in loss_names:
                ...
                # print('content_feats', content_feats)
                # print('x_feats', x_feats)
            
            if "depth_loss" in loss_names:
                print('calculating depth_loss')
                ###### calculate the depth of content image ######

                # outputs_new = outputs[0].permute(1, 2, 0).detach().cpu().numpy() * 255.0
                # opencv_output_image = cv2.cvtColor(outputs_new, cv2.COLOR_RGB2BGR)
                # # print('opencv_output_image', opencv_output_image.shape)

                # if False:
                #     imageio.imwrite(
                #         os.path.join('wenzhao', f"wenzhao_pred_midas_{str(uuid.uuid4())}.png"),
                #         np.clip(opencv_output_image, 0.0, 255.0).astype(np.uint8),
                #     )

                # input_batch = transform(opencv_output_image).to(device)
                ...
                # print('prediction', prediction)
                    ...
                    #     size=outputs[0].permute(1, 2, 0).detach().cpu().numpy().shape[:2],
                    #     mode="bicubic",
                    #     align_corners=False,
                    # ).squeeze()
                ...
                if False:
                    imageio.imwrite(
                        os.path.join('wenzhao', f"wenzhao_pred_depth_midas_{str(uuid.uuid4())}.png"),
                        pred_depth.detach().cpu().numpy()
                    )
                
                ###### calculate the depth of contents image ######
                # print('calculating depth_loss')

                # contents_new = contents[0].permute(1, 2, 0).detach().cpu().numpy() * 255.0
                # opencv_content_image = cv2.cvtColor(contents_new, cv2.COLOR_RGB2BGR)
                # # print('opencv_content_image', opencv_content_image.shape)

                # if False:
                #     imageio.imwrite(
                #         os.path.join('wenzhao', f"wenzhao_content_midas_{str(uuid.uuid4())}.png"),
                #         np.clip(opencv_content_image, 0.0, 255.0).astype(np.uint8),
                #     )

                # input_batch = transform(opencv_content_image).to(device)
                input_batch = contents
                ...
                with torch.no_grad():
                    ...
                    # prediction = torch.nn.functional.interpolate(
                    #     prediction.unsqueeze(1),
                    #     size=contents[0].permute(1, 2, 0).detach().cpu().numpy().shape[:2],
                    #     mode="bicubic",
                    #     align_corners=False,
                    # ).squeeze()

                content_depth = prediction
                # print('content_depth.shape', content_depth.shape)
                if False:
                    imageio.imwrite(
                        os.path.join('wenzhao', f"wenzhao_content_depth_map_midas_final_{str(uuid.uuid4())}.png"),
                        content_depth.detach().cpu().numpy()
                    )
                ...
                # content_feats = torch.cat([content_feats_all[ix_map[ix]] for ix in layers], 1)
                # need to generate the depth map of stylized view and origin view
                # print('x_feats.shape', x_feats.shape)
                # print('depth loss', torch.mean((content_depth / 255.0 - pred_depth / 255.0) ** 2))
                loss_dict["depth_loss"] += torch.mean((content_depth - pred_depth) ** 2)
                

            
            if "nnfm_loss" in loss_names:
                x_feats_new = x_feats.clone()
                # x_feats_new = x_feats_new.mul(mask)
                s_feats_new = s_feats.clone()
                # s_feats_new = s_feats_new.mul(mask)

                target_feats = nn_feat_replace(x_feats_new, s_feats_new)
                loss_dict["nnfm_loss"] += cos_loss(x_feats_new, target_feats)
