...
def match_colors_for_image_set(image_set, style_img_1, style_img_2):
    """
    image_set: [N, H, W, 3]
    style_img: [H, W, 3]
    """
    print('style_img_1', style_img_1.shape)
    print('style_img_2', style_img_2.shape)
...
class NNFMLoss(torch.nn.Module):
    ...
    def forward(
        ...
        with torch.no_grad():
            s_feats_all_1 = self.get_feats(styles1, all_layers)
            s_feats_all_2 = self.get_feats(styles2, all_layers)
        ...
        for block in blocks:
            ...
            s_feats_1 = torch.cat([s_feats_all_1[ix_map[ix]] for ix in layers], 1)
            s_feats_2 = torch.cat([s_feats_all_2[ix_map[ix]] for ix in layers], 1)

            if "nnfm_loss" in loss_names:
                target_feats_1 = nn_feat_replace(x_feats, s_feats_1)
                target_feats_2 = nn_feat_replace(x_feats, s_feats_2)
                loss_dict["nnfm_loss"] += cos_loss(x_feats, target_feats_1)
                loss_dict["nnfm_loss"] += cos_loss(x_feats, target_feats_2)

            if "gram_loss" in loss_names:
                loss_dict["gram_loss"] += torch.mean((gram_matrix(x_feats) - gram_matrix(s_feats_1)) ** 2)
                loss_dict["gram_loss"] += torch.mean((gram_matrix(x_feats) - gram_matrix(s_feats_2)) ** 2)
...
if __name__ == '__main__':
    ...
    fake_image_set_new, color_tf = match_colors_for_image_set(fake_image_set, fake_style1, fake_style2)
