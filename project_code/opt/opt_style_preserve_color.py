...
from nnfm_loss_preserve_color import NNFMLoss, match_colors_for_image_set
...
batch_size = None
save_new_style_img = True

nnfm_loss_fn = NNFMLoss(device=device)

print('args.disable_recolor', args.disable_recolor)
        ...
        for iter_id, batch_begin in pbar:
            ...
            if optim_type == "ray":
                ...
                def compute_image_loss():
                    ...
                    #ic('rgb_gt.shape', rgb_gt.shape)

                    #ic('style_img.shape', style_img.shape)

                    contents_temp = F.interpolate(
                            rgb_gt,
                            size=None,
                            scale_factor=0.5,
                            mode="bilinear",
                    ...
                    # torch.save(contents_temp, 'contents_temp_wenzhao.pt')
                    # torch.save(style_img, 'style_img_wenzhao.pt')
                    # histogram matching between content and style
                    convert_style_image = style_img.cpu().detach().numpy()*255
                    convert_content_image = contents_temp[0].cpu().permute(1, 2, 0).detach().numpy() * 255.0
                    
                    matched = match_histograms(convert_style_image, convert_content_image, channel_axis=-1)
                    matched = matched / 255.0

                    global save_new_style_img
                    if save_new_style_img:
                        imageio.imwrite(
                            os.path.join(args.train_dir, f"convert_content_image{img_id}_{epoch_id}.png"),
                            np.clip(matched * 255.0, 0.0, 255.0).astype(np.uint8),
                        )
                        save_new_style_img = False


                    matched_style_img = torch.from_numpy(matched).to(device)
                    ...
                    loss_dict = nnfm_loss_fn(
                        ...
                        # here we use the matched-style images to preserve color
                        matched_style_img.permute(2, 0, 1).unsqueeze(0),
                        blocks=[
                            args.vgg_block,
                        ],
