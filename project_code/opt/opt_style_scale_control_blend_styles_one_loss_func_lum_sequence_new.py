...
from nnfm_loss_scale_control_blend_styles_one_loss_func_lum_sequence_new import NNFMLoss, match_colors_for_image_set
...
ic("Style image: ", args.style1, style_img.shape)
...
style_img = torch.from_numpy(style_img).to(device=device)
ic("Style image: ", args.style2, style_img.shape)
...
nnfm_loss_fn_two_style = NNFMLoss(device=device)

while True:
    def train_step(optim_type, epoch_id):
        ...
        for iter_id, batch_begin in pbar:
            ...
            if optim_type == "ray":
                ...
                def compute_image_loss():
                    ...
                    loss_dict = nnfm_loss_fn_two_style(
                        F.interpolate(
                            rgb_pred,
                            size=None,
                            scale_factor=0.5,
                    ...
                    # loss_dict["nnfm_loss"] *= 5 # add by wenzhao because the luminance 
    ...
    if epoch_id < args.mse_num_epoches:
        ...
        train_step(optim_type="ray", epoch_id = epoch_id)
        ...
        train_step(optim_type="image", epoch_id = epoch_id)
        # color preserve
        if True:
            num_views, view_height, view_width = dset.n_images, dset.h, dset.w
