...
from nnfm_loss_scale_control_formula_blend_styles import NNFMLoss, match_colors_for_image_set
...
# parser.add_argument("--scale_value", type=float, default=1, help="scale_value to control the style image size")
# parser.add_argument("--style_vgg_layer", type=str, default='0-1-2-3-4', help="vgg style layer index for scale control")
parser.add_argument("--style1_img_scales", type=str, default='1-1-1-1-1', help="style1-scale value for style img for each layers fine-tune for scale control")
parser.add_argument("--style2_img_scales", type=str, default='1-1-1-1-1', help="style2-scale value for style img for each layers fine-tune for scale control")
parser.add_argument("--style1_vgg_layer_weight", type=str, default='0-0-0.7-0.3-0', help="style1-vgg style layer weights for scale control")
parser.add_argument("--style2_vgg_layer_weight", type=str, default='0.3-0.7-0-0-0', help="style2-vgg style layer weights for scale control")
parser.add_argument(
    "--vgg_block",
    type=int,
    default=2,
...
while True:
        ...
        for iter_id, batch_begin in pbar:
            ...
            if optim_type == "ray":
                ...
                def compute_image_loss():
                    ...
                    # print('args.style_vgg_layer', args.style_vgg_layer)
                    print('args.style_vgg_layer_weight', args.style_vgg_layer_weight)
                    print('args.style_img_scales', args.style_img_scales)
                    loss_dict = nnfm_loss_fn(
                        ...
                        # style_vgg_layer=args.style_vgg_layer,
                        style1_vgg_layer_weight=args.style1_vgg_layer_weight,
                        style1_img_scales=args.style1_img_scales,
                        style2_vgg_layer_weight=args.style2_vgg_layer_weight,
                        style2_img_scales=args.style2_img_scales,
                    )
    ...
    if epoch_id < args.mse_num_epoches:
        ...
        if False:
            ...
            # print('before', dset.rays.gt)
            rgb_gt_pre = copy.deepcopy(rgb_gt)
            # print('before', rgb_gt_pre)
            with torch.no_grad():
            ...
            # dset.rays.gt, color_tf = match_colors_for_image_set(dset.rays.gt, rgb_gt_pre)
            # grid.apply_ct(color_tf.detach().cpu().numpy())
    ...
    if epoch_id >= args.mse_num_epoches + args.nnfm_num_epoches:
        print('args.no_post_ct', args.no_post_ct)
        if not args.no_post_ct:
            ...
            with torch.no_grad():
                for img_id in range(num_views):
                    ...
                    rgb_pred = grid.volume_render_image(cam, use_kernel=True)
                    rgb_gt[img_id] = rgb_pred.reshape(view_height, view_width, 3).contiguous().cpu().clamp_(0.0, 1.0)
                    print(rgb_gt[img_id].shape)
                    print('img_id', img_id)
            
            # print('after', rgb_gt)
            # print('after-shape', rgb_gt.shape)
            # print('before', rgb_gt_pre)
            # print('before-shape', rgb_gt_pre.shape)
            # print('after', dset.rays.gt) # 是换成rgb_pred之后的
            # dset.rays.gt, color_tf = match_colors_for_image_set(dset.rays.gt, style_img)
            # for img_id in range(num_views):
            #     #.view(-1, 3)
            #     dset.rays.gt, color_tf = match_colors_for_image_set(dset.rays.gt, rgb_gt_pre[img_id])
            #     grid.apply_ct(color_tf.detach().cpu().numpy())
