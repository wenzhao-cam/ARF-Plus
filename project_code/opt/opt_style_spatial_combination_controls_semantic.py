...
from nnfm_loss import NNFMLoss as NNFMLoss_origin

# from nnfm_loss_spatial_control_blend_styles import NNFMLoss
from nnfm_loss_color_control_luminance import NNFMLoss as NNFMLoss_color_control_lum

from nnfm_loss_color_control_lum_and_scale_control import NNFMLoss as NNFMLoss_scale_and_lum
...
nnfm_loss_fn_scale_and_lum = NNFMLoss_scale_and_lum(device=device)
        ...
        for iter_id, batch_begin in pbar:
            ...
            if optim_type == "ray":
                ...
                def compute_image_loss():
                    ...
                    #nnfm_loss_fn_scale_and_lum

                    # loss_dict = nnfm_loss_fn(
                    loss_dict = nnfm_loss_fn_scale_and_lum(
                        F.interpolate(
                            rgb_pred,
                            size=None,
                            scale_factor=0.5,
                    ...
                    # if epoch_id == 3:
                    #     imageio.imwrite(
                    #         os.path.join(args.train_dir, f"origin_depth_map_{img_id:04d}_final.png"),
                    #         depth_img.detach().cpu().numpy()
                    #     )    
                    ...
                    if False: # sequence disable the first when rendering style1
                        rgb_pred.grad *= mask
                    ############### use mask to control grad changes #############
                ...
                def compute_image_loss():
                    ...
                    # if False: # use to indicate whether to reverse the depth map
                    ...
                    # print('rgb_pred', rgb_pred.shape) # rgb_pred torch.Size([1, 3, 756, 1008])

                    # print('rgb_gt.shape wzzz', rgb_gt.shape)
                    if True:
                        ...
                        if epoch_id == 3:
                            imageio.imwrite(
                                os.path.join(args.train_dir, f"semantic_seg_map_{img_id:04d}.png"),
