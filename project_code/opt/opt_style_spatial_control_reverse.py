...
from nnfm_loss_spatial_control_reverse import NNFMLoss, match_colors_for_image_set
        ...
        for iter_id, batch_begin in pbar:
            ...
            if optim_type == "ray":
                ...
                def compute_image_loss():
                    ...
                    # loss_dict["img_tv_loss"] = img_tv_loss * 100  # add by wenzhao
                    ...
                    # create mask
                    #
                    ############### binary mask half zeros #############
                    mask_height = rgb_pred.grad.shape[2]
                    mask_width = rgb_pred.grad.shape[3]
                    mask_bath_size = rgb_pred.grad.shape[1]

                    mask = np.ones((mask_height, mask_width))
                    mask[mask_height//2:, :] = 0
                    ...
                    image_result_display_reverted =  np.abs(image_result.astype(np.uint8) // 255 - 1)

                    if epoch_id == 3:
                        imageio.imwrite(
                            os.path.join(args.train_dir, f"depth_thred_map_{img_id:04d}.png"),
                            # image_result.astype(np.uint8),
                            image_result_display_reverted.astype(np.uint8),
                        )
                    
                    # print('image_result', image_result.astype(np.uint8) / 255.0)
                    # print('depth_img.shape', depth_img.shape)
                    # print('depth_img', depth_img)
                    ...
                    if False:
                        imageio.imwrite(
                            os.path.join(args.train_dir, f"depth_thred_map_{img_id:04d}.png"),
                            # image_result.astype(np.uint8) * 255.0,
                            np.clip(image_result * 255.0, 0.0, 255.0).astype(np.uint8),
                        )
                    ...
                    depth_img = torch.from_numpy(mask_array)
                    # ======================== 

                    # print('image_result', depth_img)
                    # depth_img = depth_img.permute(2, 0, 1)

                    # print('depth_img.shape', depth_img.shape)
                    # print('depth_img', depth_img)

                    # mask = depth_img[None].to(device=device)
                    ...
                    # print('rgb_pred.grad.shape', rgb_pred.grad.shape)
