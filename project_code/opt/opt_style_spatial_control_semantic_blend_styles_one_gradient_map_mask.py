    ...
    def train_step(optim_type):
        ...
        for iter_id, batch_begin in pbar:
            ...
            if optim_type == "ray":
                ...
                def compute_image_loss_style_1():
                    with torch.no_grad():
                        cam = svox2.Camera(
                ...
                def compute_image_loss_style_2():
                    with torch.no_grad():
                        cam = svox2.Camera(
                            dset.c2w[img_id].to(device=device),
                            dset.intrins.get("fx", img_id),
                ...
                rgb_pred_grad_style_1, loss_dict_style_1 = compute_image_loss_style_1()
                rgb_pred_grad_style_2, loss_dict_style_2 = compute_image_loss_style_2()
                rgb_pred_grad = rgb_pred_grad_style_1 + rgb_pred_grad_style_2

                # for x in loss_dict_style_1:
                #     loss_dict[x] = loss_dict_style_1[x] + 

                # loss_dict = loss_dict_style_1
                ...
                #     #add by wenzhao to test https://discuss.pytorch.org/t/set-requires-grad-according-to-a-binary-mask/153040/2
                #     # set requires_grad according to a binary mask
                #     # with torch.no_grad():
                #     #     rgb_pred = saved_rgb_pred
                #     #add by wenzhao
                #     # print('rgb_pred', rgb_pred.shape)
                #     # print('rgb_pred.grad', rgb_pred.grad.shape)
                #     # print('saved_rgb_pred', saved_rgb_pred)
                #     # print('saved_rgb_pred.grad', saved_rgb_pred.grad)
                #     # create mask
                #     ############### depth mask start #############
                #     depth_img = grid.volume_render_depth_image(cam,
                #         args.log_depth_map_use_thresh if
                ...
                #     if False:
                #         imageio.imwrite(
                #             os.path.join(args.train_dir, f"depth_thred_map_{img_id:04d}_{epoch_id}.png"),
                #             image_result.astype(np.uint8),
                #         )
                    
                #     image_result = image_result.astype(np.uint8) // 255
                #     # image_result = np.abs(image_result - 1)
                #     depth_img = torch.tensor(image_result)
                #     # depth_img = torch.tensor(image_result.astype(np.uint8) // 255)
                ...
                #     # masked_image_array =  np.expand_dims(mask_array, axis=2)
                #     # # Convert the masked image array back to PIL Image
                #     # masked_image = Image.fromarray(masked_image_array.astype(np.uint8))
                #     # # Save the masked image
                #     # if True: 
                #     #     output_path = os.path.join(photo_in_dir, f"mask_{img_id:04d}.png")
                #     #     masked_image.save(output_path)
                #     depth_img = torch.from_numpy(mask_array.astype(int))
                #     # ======================== 

                #     mask = depth_img.unsqueeze(0).repeat(3, 1, 1)
                #     mask = mask[None].to(device=device)
                ...
                for x in loss_dict_style_1:
                    stats[x] = loss_dict_style_1[x].item() + loss_dict_style_2[x].item()
