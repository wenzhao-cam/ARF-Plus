...
from nnfm_loss_two_style import match_colors_for_image_set
from nnfm_loss_spatial_control_blend_styles import NNFMLoss
...
parser.add_argument("--style1", type=str, help="path to style image")
parser.add_argument("--style2", type=str, help="path to style image")
parser.add_argument("--content_weight", type=float, default=5e-3, help="content loss weight")
parser.add_argument("--semantic_type", type=str, default="room", help="semantic segmentation category")
...
style_img = imageio.imread(args.style1).astype(np.float32) / 255.0
style_h, style_w = style_img.shape[:2]
content_long_side = max([dset.w, dset.h])
content_short_side = min([dset.w, dset.h])
...
#     style_img = cv2.resize(
#         style_img,
#         (content_short_side, content_long_side),
#         interpolation=cv2.INTER_AREA,
#     )
# else:
#     style_img = cv2.resize(
#         style_img,
#         (content_long_side, content_short_side),
#         interpolation=cv2.INTER_AREA,
#     )
style_img = cv2.resize(
    style_img,
...
imageio.imwrite(
    os.path.join(args.train_dir, "style_image1.png"),
    np.clip(style_img * 255.0, 0.0, 255.0).astype(np.uint8),
)
style_img = torch.from_numpy(style_img).to(device=device)
style_img1 = style_img
ic("Style image: ", args.style1, style_img1.shape)

###### resize style image such that its long side matches the long side of content images
style_img = imageio.imread(args.style2).astype(np.float32) / 255.0
style_h, style_w = style_img.shape[:2]
content_long_side = max([dset.w, dset.h])
content_short_side = min([dset.w, dset.h])
...
imageio.imwrite(
    os.path.join(args.train_dir, "style_image2.png"),
    np.clip(style_img * 255.0, 0.0, 255.0).astype(np.uint8),
)
style_img = torch.from_numpy(style_img).to(device=device)
style_img2 = style_img
ic("Style image: ", args.style2, style_img2.shape)
...
#     dset.rays.gt, color_tf = match_colors_for_image_set(dset.rays.gt, style_img1, style_img2)
...
while True:
    def train_step(optim_type, stylize_type=0):
        ...
        for iter_id, batch_begin in pbar:
            ...
            if optim_type == "ray":
                ...
                stats["psnr"] = psnr
            elif optim_type == "image" and stylize_type == 0:
                ################# style 1 training ################# 
                ...
                def compute_image_loss():
                    with torch.no_grad():
                        ...
                        if args.log_depth_map:
                            ...
                            imageio.imwrite(
                                os.path.join(args.train_dir, f"depth_map_{img_id:04d}_{epoch_id}_final.png"),
                                depth_img,
                            )
                    ...
                    # https://stackoverflow.com/questions/57668368/how-to-create-torch-tensor-object-and-to-update-only-some-of-its-elements
                    # rgb_pred.requires_grad_(True)

                    rgb_pred.requires_grad_(True)
                    # saved_rgb_pred = rgb_pred.clone()
                    # saved_rgb_pred.requires_grad_(False)
                    # with torch.no_grad():
                    #     saved_rgb_pred = rgb_pred.clone()
                    #     print('saved_rgb_pred.shape', saved_rgb_pred.shape)
                    ...
                    if True:
                        ...
                        #semantic_img = torch.from_numpy(mask_array.astype(int))
                        semantic_mask = semantic_img.unsqueeze(0).repeat(3, 1, 1)
                        semantic_mask = semantic_mask[None].to(device=device)

        
                    ############### 

                    # print('semantic_mask shape', semantic_mask.shape) # semantic_mask shape torch.Size([1, 3, 546, 980])
                    # print('rgb_pred shape', rgb_pred.shape) #rgb_pred shape torch.Size([1, 3, 546, 980])

                    if False:
                        rgb_pred_temp = torch.mul(rgb_pred, semantic_mask)  
                        
                        # print('rgb_pred', rgb_pred)
                        # rgb_gt_temp = copy.deepcopy(rgb_gt) * semantic_mask 
                        rgb_gt_temp = torch.mul(copy.deepcopy(rgb_gt), semantic_mask) 
                    else:
                        rgb_pred_temp = rgb_pred
                        rgb_gt_temp = rgb_gt
                    ...
                    loss_dict = nnfm_loss_fn(
                        ...
                        style_img1.permute(2, 0, 1).unsqueeze(0),
                        blocks=[
                            args.vgg_block,
                        ],
                    ...
                    # print('rgb_pred.gra', rgb_pred.grad)
                    # print('semantic_mask shape', semantic_mask.shape)

                    ############### use mask to control grad changes #############
                    if True:
                        rgb_pred.grad *= semantic_mask
                    ############### use mask to control grad changes #############
                ...
                if iter_id == 10:
                    rgb_pred_display = rgb_pred.detach().cpu().numpy()

                    imageio.imwrite(
                        os.path.join(args.train_dir, f"logims_{img_id:04d}_{epoch_id}_style_1.png"),
                        np.clip(rgb_pred_display * 255.0, 0.0, 255.0).astype(np.uint8),
                    )

            ################# semantic area back to original training ################# 
            elif optim_type == "image" and stylize_type == 1:
                num_views, view_height, view_width = dset.n_images, dset.h, dset.w
                img_id = np.random.randint(low=0, high=num_views)
                rays = svox2.Rays(
                    dset.rays.origins.view(num_views, view_height * view_width, 3)[img_id].to(device),
                ...
                def compute_image_loss():
                    with torch.no_grad():
                        ...
                        if args.log_depth_map:
                            ...
                            # print('depth_img.shape', depth_img.shape)
                            # print('depth_img', depth_img)
                            # im = depth_img.cpu().numpy()
                            # im = (im * 255).astype(np.uint8)
                            # img_path = path.join(render_dir, f'depth_map_{img_id:04d}.png');
                            # imageio.imwrite(img_path,im)
                            # rgb_pred = grid.volume_render_image(cam, use_kernel=True)
                            # depth_img = rgb_pred.detach().cpu().numpy()
                    ...
                    loss_dict = nnfm_loss_fn(
                        ...
                        style_img2.permute(2, 0, 1).unsqueeze(0),
                        blocks=[
                            args.vgg_block,
                        ],
                        loss_names=["content_loss"], # only content loss
                        contents=F.interpolate(
                            # rgb_gt,
                            rgb_gt_temp,
                            size=None,
                    ...
                    # loss_dict["content_loss"] *= args.content_weight
                    # loss_dict["content_loss"] *= 100 # add by Wenzhao learn back content_weight

                    loss_dict["img_tv_loss"] = img_tv_loss
                    loss_dict["img_tv_loss"] *= 100 # add by wenzhao
                    loss = sum(list(loss_dict.values()))
                    loss.backward()
                ...
                if iter_id == 10:
                    ...
                    imageio.imwrite(
                        os.path.join(args.train_dir, f"logim_{img_id:04d}_{epoch_id}_back_orig.png"),
                        np.clip(rgb_pred_display * 255.0, 0.0, 255.0).astype(np.uint8),
                    )

              
            ################# style 2 training ################# 
            elif optim_type == "image" and stylize_type == 2:  
                num_views, view_height, view_width = dset.n_images, dset.h, dset.w
                img_id = np.random.randint(low=0, high=num_views)
                rays = svox2.Rays(
                    dset.rays.origins.view(num_views, view_height * view_width, 3)[img_id].to(device),
                ...
                def compute_image_loss():
                    ...
                    if True:
                        ...
                        if True:
                            ...
                            mask_array = np.logical_not(mask_array)
                            semantic_img = mask_array
                            # semantic_img_reverted = np.logical_not(mask_array)
                        #===========end=================
                        
                        semantic_img_reverted = np.logical_not(semantic_img)
                        ...
                        semantic_img_reverted = torch.tensor(semantic_img_reverted)
                        semantic_mask_reverted = semantic_img_reverted.unsqueeze(0).repeat(3, 1, 1)
                        semantic_mask_reverted = semantic_mask_reverted[None].to(device=device)
                    
          
                    rgb_pred_temp = torch.mul(rgb_pred, semantic_mask)  
                    rgb_gt_temp_reverted = torch.mul(copy.deepcopy(rgb_gt), semantic_mask_reverted)
                    
                    if False:
                        # print('rgb_pred_temp', rgb_pred_temp.shape)
                        # print('rgb_gt_temp_reverted', rgb_gt_temp_reverted.shape)
                        
                        rgb_pred_total = rgb_pred_temp + rgb_gt_temp_reverted
                        # print('rgb_pred', rgb_pred)
                        # rgb_gt_temp = copy.deepcopy(rgb_gt) * semantic_mask 
                    else: 
                        rgb_pred_total = rgb_pred_temp
                ...
                if iter_id == 10:
                    ...
                    imageio.imwrite(
                        os.path.join(args.train_dir, f"logim_{img_id:04d}_{epoch_id}_style_2.png"),
                        np.clip(rgb_pred_display * 255.0, 0.0, 255.0).astype(np.uint8),
                    )
    ...
    # then train the origin content in semantic region back select_type=1
    # elif epoch_id < args.mse_num_epoches + 4 +  args.nnfm_num_epoches: 
    elif epoch_id < args.mse_num_epoches + 2 * args.nnfm_num_epoches: # control content back
        epoch_size = dset.n_images
        batch_size = 1
        batches_per_epoch = (dset.n_images - 1) // batch_size + 1
        train_step(optim_type="image", stylize_type=1)

    
    # then train the semantic region with style2 select_type=2
    # elif epoch_id >= args.mse_num_epoches + 4 +  args.nnfm_num_epoches and  epoch_id < args.mse_num_epoches + 3 + 2 * args.nnfm_num_epoches:
    elif epoch_id >= args.mse_num_epoches + 2 * args.nnfm_num_epoches and  epoch_id < args.mse_num_epoches + 3 * args.nnfm_num_epoches:
        epoch_size = dset.n_images
        batch_size = 1
        batches_per_epoch = (dset.n_images - 1) // batch_size + 1
        train_step(optim_type="image", stylize_type=2)
    ...
    # if epoch_id >= args.mse_num_epoches + 4 + 2 * args.nnfm_num_epoches:
    if epoch_id >= args.mse_num_epoches + 3 * args.nnfm_num_epoches:
