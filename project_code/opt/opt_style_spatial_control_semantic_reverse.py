            ...
            if optim_type == "ray":
                ...
                def compute_image_loss():
                    ...
                    if True:
                        ...
                        # print('semantic_img', semantic_img)
                        # print('semantic_img max', max(max(row) for row in semantic_img))
                        ##### reverse mask #####
                        # semantic_img = np.abs(semantic_img - 1)
                        if False:
                            imageio.imwrite(
                        ...
                        if True:
                            image_result_display_reverted =  np.abs(semantic_img.astype(np.uint8) - 1)
                            imageio.imwrite(
                                os.path.join(args.train_dir, f"semantic_seg_map_{img_id:04d}.png"),
                        ...
                        # semantic_img = [[int(not bool(element)) for element in row] for row in semantic_img]
                        # semantic_img = np.abs(np.subtract(semantic_img, 1))
