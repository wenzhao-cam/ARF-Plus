...
from nnfm_loss_test_MiDaS_depth import NNFMLoss, match_colors_for_image_set
...
parser.add_argument("--depth_weight", type=float, default=5e-3, help="depth loss weight")
            ...
            if optim_type == "ray":
                ...
                def compute_image_loss():
                    with torch.no_grad():
                        ...
                        rgb_pred = grid.volume_render_image(cam, use_kernel=True)
                        rgb_pred_save = rgb_pred.detach().cpu().numpy()
                        if False:
                            imageio.imwrite(
                                os.path.join(args.train_dir, f"rgb_pred{img_id:04d}_{epoch_id}.png"),
                                np.clip(rgb_pred_save * 255.0, 0.0, 255.0).astype(np.uint8),
                            )
                        ...
                        ######### preview current rgb_pred add by wenzhao ##########
                    ...
                    loss_dict = nnfm_loss_fn(
                        ...
                        loss_names=["nnfm_loss", "content_loss", "depth_loss"],
                    ...
                    print('origin loss_dict', loss_dict)

                    loss_dict["content_loss"] *= args.content_weight
                    loss_dict["depth_loss"] *= args.depth_weight
                    loss_dict["img_tv_loss"] = img_tv_loss

                    print('weighted loss_dict', loss_dict)
